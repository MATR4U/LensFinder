...
- **Health/Readiness** [Status: Done]
- Implemented `/ready` (checks DB connectivity) and `/api/health` (liveness) with `HEAD` for health; wired probes in `infra/k8s/server.yaml`.
