...
export type TieBreaker = { path: Path; dir: 'asc' | 'desc' };
...
export type Executable = {
  ...
  score: (item: unknown) => { total: number; parts: number[] };
  clauses: Clause[];
};

export type Explanation = {
  pass: boolean;
  parts: Array<{ clause: Clause; pass: boolean; contribution: number }>;
  total: number;
};

export type ValidationError = { message: string; at?: string };
