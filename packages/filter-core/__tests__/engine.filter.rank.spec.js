...
describe('filter/rank', () => {
    it('filters with hard constraints', () => {
        const spec = { allOf: [{ path: 'a', op: 'gte', value: 2, mode: 'hard' }] };
        const out = filter(data, spec);
        expect(out.map(x => x.id)).toEqual([2, 3]);
    });
    it('ranks with soft constraints', () => {
        const spec = { allOf: [{ path: 'a', op: 'gte', value: 1, mode: 'hard' }, { path: 'a', op: 'lte', value: 3, mode: 'soft', weight: 1 }] };
        const out = rank(data, spec, { topK: 2 });
        expect(out.length).toBe(2);
    });
    it('compile and explain parts consistency', () => {
        const spec = { allOf: [{ path: 'b', op: 'includes', value: 'al', mode: 'hard' }] };
