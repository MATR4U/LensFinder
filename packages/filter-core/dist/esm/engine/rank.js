...
export async function rankAsync(iter, execOrSpec, options = {}) {
    const exec = execOrSpec.test ? execOrSpec : compile(execOrSpec);
    const topK = typeof options.topK === 'number' && options.topK > 0 ? new TopK(options.topK) : undefined;
    const res = [];
...
export function rank(data, execOrSpec, options = {}) {
