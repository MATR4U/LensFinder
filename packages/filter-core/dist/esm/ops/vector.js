export const OPS = ['cosineSimGte', 'dotGte', 'l2DistLte'];
function dot(a, b) {
    let s = 0;
...
export function cmpFactory(op, value) {
    if (op === 'cosineSimGte') {
        ...
        return (x) => Array.isArray(x) && cosineSim(x, v) >= thr;
    }
    if (op === 'dotGte') {
        const v = Array.isArray(value?.vector) ? value.vector : value;
        const thr = typeof value?.threshold === 'number' ? value.threshold : 0;
        return (x) => Array.isArray(x) && dot(x, v) >= thr;
    }
    if (op === 'l2DistLte') {
        const v = Array.isArray(value?.vector) ? value.vector : value;
        const thr = typeof value?.threshold === 'number' ? value.threshold : 0;
        return (x) => Array.isArray(x) && l2Dist(x, v) <= thr;
    }
    return () => false;
}
