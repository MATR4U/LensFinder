...
const USE_CASE_WEIGHTS: Record<string, Priorities> = {
...
function averageUseCaseWeights(useCases: string[]): Priorities {
  const cases = (useCases || []).filter((c) => USE_CASE_WEIGHTS[c])
  ...
  const sum = cases.reduce((acc, c) => {
    ...
    acc.zoom += w.zoom
    return acc
  }, { portability: 0, lowLight: 0, zoom: 0 } as Priorities)
  return { portability: sum.portability / cases.length, lowLight: sum.lowLight / cases.length, zoom: sum.zoom / cases.length }
}
...
export async function getRecommendations(payload: RecommendationRequest): Promise<ScoredLens[]> {
  ...
  const scored = list.map((r) => {
    ...
    const item: ScoredLens = {
      ...
      price: r.price_chf ?? 0,
      weight: r.weight_g ?? 0,
      minFocalLength: r.focal_min_mm ?? 0,
      maxFocalLength: r.focal_max_mm ?? 0,
      maxAperture: r.aperture_min ?? 0,
  ...
  if (budget?.isFlexible && Number.isFinite(budget?.target)) {
    ...
    candidates = [...under, ...overGood]
  } else if (Number.isFinite(budget?.target)) {
    const hard = Math.max(0, Math.floor(budget.target))
    candidates = candidates.filter((c) => c.price == null || (c.price as number) <= hard)
  }

  candidates.sort((a, b) => (b.overallScore - a.overallScore) || ((a.price ?? Infinity) as number - (b.price ?? Infinity) as number))
