...
import { ticksFromBounds } from '../lib/formatters';
import { getUnitFormatters } from '../lib/units';
...
export function useMetricConfig(metric: Metric) {
  ...
  if (metric === 'price') {
    ...
    return {
      ...
      step: 50,
      unit: 'CHF' as const,
      tickFormatter: getUnitFormatters('CHF').tickFormatter,
    };
  }
  if (metric === 'weight') {
    ...
    return {
      ...
      step: 10,
      unit: 'g' as const,
      tickFormatter: getUnitFormatters('g').tickFormatter,
    };
  }
  if (metric === 'distortion') {
    ...
    return {
      ...
      step: 0.1,
      unit: '%' as const,
      tickFormatter: getUnitFormatters('%').tickFormatter,
    };
  }
  if (metric === 'breathing') {
    ...
    return {
      ...
      step: 0.5,
      unit: undefined,
      tickFormatter: getUnitFormatters('mm').tickFormatter,
    };
  }
  // focal
