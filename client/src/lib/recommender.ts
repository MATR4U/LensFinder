...
export const DEFAULT_STOPS_IF_OIS = 3.5;

export function isZoom(lens: Lens) { return lens.focal_min_mm !== lens.focal_max_mm; }

export function maxApertureAt(lens: Lens, focal_mm: number) {
  if (!isZoom(lens) || Math.abs(lens.aperture_min - lens.aperture_max) < 1e-6) return lens.aperture_min;
...
export function scoreLens(lens: Lens, camera: Camera, goal_weights: Record<string, number>, focal_choice_mm: number) {
  const maxAperture = maxApertureAt(lens, focal_choice_mm);
  let low_light = optics.lightGatheringScore(focal_choice_mm, maxAperture);
  if (lens.ois || camera.ibis) low_light *= optics.stabilizationBonus(DEFAULT_STOPS_IF_OIS);
  ...
  const raw: Record<string, number> = { low_light, background_blur, reach, wide, portability, value, distortion_control, video_excellence };
  let total = 0;
  for (const key in raw) total += (raw[key] || 0) * (goal_weights[key] || 0.0);
  raw.total = total;
  return raw as Record<string, number> & { total: number };
}
...
export function computeResults(params: {
  ...
  return lenses.map((lens) => {
    ...
    return {
      ...
      ...scores,
      focal_used_mm: fc,
      max_aperture_at_focal: maxApertureAt(lens, fc),
      eq_focal_ff_mm: optics.equivFocalFf(fc, camera.sensor),
      fov_h_deg: h,
