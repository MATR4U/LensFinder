...
export default function CheckboxGroup({ label, infoText, items, layout = 'row', hint, status, showItemLabels = true }: Props) {
  ...
  return (
    <FieldContainer label={label} info={infoText ? <Info text={infoText} /> : undefined} hint={hint} status={status} labelId={groupLabelId}>
      <div className={layout === 'row' ? 'flex flex-wrap gap-4 items-center' : 'flex flex-col gap-2'} aria-labelledby={groupLabelId}>
        {items.map((item) => {
          ...
          return (
            <label
              ...
              htmlFor={checkboxId}
              className="inline-flex items-center gap-2 text-sm text-[var(--text-muted)] cursor-pointer select-none"
            >
